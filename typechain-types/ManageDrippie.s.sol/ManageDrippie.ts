/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import type {
  BaseContract,
  BigNumber,
  BytesLike,
  CallOverrides,
  ContractTransaction,
  Overrides,
  PopulatedTransaction,
  Signer,
  utils,
} from "ethers";
import type { FunctionFragment, Result } from "@ethersproject/abi";
import type { Listener, Provider } from "@ethersproject/providers";
import type {
  TypedEventFilter,
  TypedEvent,
  TypedListener,
  OnEvent,
} from "../common";

export interface ManageDrippieInterface extends utils.Interface {
  functions: {
    "IS_SCRIPT()": FunctionFragment;
    "cfg()": FunctionFragment;
    "installDrips()": FunctionFragment;
    "pauseDrips()": FunctionFragment;
    "run()": FunctionFragment;
    "setUp()": FunctionFragment;
  };

  getFunction(
    nameOrSignatureOrTopic:
      | "IS_SCRIPT"
      | "cfg"
      | "installDrips"
      | "pauseDrips"
      | "run"
      | "setUp"
  ): FunctionFragment;

  encodeFunctionData(functionFragment: "IS_SCRIPT", values?: undefined): string;
  encodeFunctionData(functionFragment: "cfg", values?: undefined): string;
  encodeFunctionData(
    functionFragment: "installDrips",
    values?: undefined
  ): string;
  encodeFunctionData(
    functionFragment: "pauseDrips",
    values?: undefined
  ): string;
  encodeFunctionData(functionFragment: "run", values?: undefined): string;
  encodeFunctionData(functionFragment: "setUp", values?: undefined): string;

  decodeFunctionResult(functionFragment: "IS_SCRIPT", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "cfg", data: BytesLike): Result;
  decodeFunctionResult(
    functionFragment: "installDrips",
    data: BytesLike
  ): Result;
  decodeFunctionResult(functionFragment: "pauseDrips", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "run", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "setUp", data: BytesLike): Result;

  events: {};
}

export interface ManageDrippie extends BaseContract {
  connect(signerOrProvider: Signer | Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: ManageDrippieInterface;

  queryFilter<TEvent extends TypedEvent>(
    event: TypedEventFilter<TEvent>,
    fromBlockOrBlockhash?: string | number | undefined,
    toBlock?: string | number | undefined
  ): Promise<Array<TEvent>>;

  listeners<TEvent extends TypedEvent>(
    eventFilter?: TypedEventFilter<TEvent>
  ): Array<TypedListener<TEvent>>;
  listeners(eventName?: string): Array<Listener>;
  removeAllListeners<TEvent extends TypedEvent>(
    eventFilter: TypedEventFilter<TEvent>
  ): this;
  removeAllListeners(eventName?: string): this;
  off: OnEvent<this>;
  on: OnEvent<this>;
  once: OnEvent<this>;
  removeListener: OnEvent<this>;

  functions: {
    IS_SCRIPT(overrides?: CallOverrides): Promise<[boolean]>;

    /**
     * Drippie configuration.
     */
    cfg(overrides?: CallOverrides): Promise<[string]>;

    /**
     * Installs drips in the drippie contract.
     */
    installDrips(
      overrides?: Overrides & { from?: string }
    ): Promise<ContractTransaction>;

    /**
     * Pauses drips that have been removed from config.
     */
    pauseDrips(
      overrides?: Overrides & { from?: string }
    ): Promise<ContractTransaction>;

    /**
     * Runs the management script.
     */
    run(
      overrides?: Overrides & { from?: string }
    ): Promise<ContractTransaction>;

    /**
     * Sets up the deployment script.
     */
    setUp(
      overrides?: Overrides & { from?: string }
    ): Promise<ContractTransaction>;
  };

  IS_SCRIPT(overrides?: CallOverrides): Promise<boolean>;

  /**
   * Drippie configuration.
   */
  cfg(overrides?: CallOverrides): Promise<string>;

  /**
   * Installs drips in the drippie contract.
   */
  installDrips(
    overrides?: Overrides & { from?: string }
  ): Promise<ContractTransaction>;

  /**
   * Pauses drips that have been removed from config.
   */
  pauseDrips(
    overrides?: Overrides & { from?: string }
  ): Promise<ContractTransaction>;

  /**
   * Runs the management script.
   */
  run(overrides?: Overrides & { from?: string }): Promise<ContractTransaction>;

  /**
   * Sets up the deployment script.
   */
  setUp(
    overrides?: Overrides & { from?: string }
  ): Promise<ContractTransaction>;

  callStatic: {
    IS_SCRIPT(overrides?: CallOverrides): Promise<boolean>;

    /**
     * Drippie configuration.
     */
    cfg(overrides?: CallOverrides): Promise<string>;

    /**
     * Installs drips in the drippie contract.
     */
    installDrips(overrides?: CallOverrides): Promise<void>;

    /**
     * Pauses drips that have been removed from config.
     */
    pauseDrips(overrides?: CallOverrides): Promise<void>;

    /**
     * Runs the management script.
     */
    run(overrides?: CallOverrides): Promise<void>;

    /**
     * Sets up the deployment script.
     */
    setUp(overrides?: CallOverrides): Promise<void>;
  };

  filters: {};

  estimateGas: {
    IS_SCRIPT(overrides?: CallOverrides): Promise<BigNumber>;

    /**
     * Drippie configuration.
     */
    cfg(overrides?: CallOverrides): Promise<BigNumber>;

    /**
     * Installs drips in the drippie contract.
     */
    installDrips(overrides?: Overrides & { from?: string }): Promise<BigNumber>;

    /**
     * Pauses drips that have been removed from config.
     */
    pauseDrips(overrides?: Overrides & { from?: string }): Promise<BigNumber>;

    /**
     * Runs the management script.
     */
    run(overrides?: Overrides & { from?: string }): Promise<BigNumber>;

    /**
     * Sets up the deployment script.
     */
    setUp(overrides?: Overrides & { from?: string }): Promise<BigNumber>;
  };

  populateTransaction: {
    IS_SCRIPT(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    /**
     * Drippie configuration.
     */
    cfg(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    /**
     * Installs drips in the drippie contract.
     */
    installDrips(
      overrides?: Overrides & { from?: string }
    ): Promise<PopulatedTransaction>;

    /**
     * Pauses drips that have been removed from config.
     */
    pauseDrips(
      overrides?: Overrides & { from?: string }
    ): Promise<PopulatedTransaction>;

    /**
     * Runs the management script.
     */
    run(
      overrides?: Overrides & { from?: string }
    ): Promise<PopulatedTransaction>;

    /**
     * Sets up the deployment script.
     */
    setUp(
      overrides?: Overrides & { from?: string }
    ): Promise<PopulatedTransaction>;
  };
}

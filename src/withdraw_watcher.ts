...
import {L1CrossDomainMessenger__factory, L1StandardBridge__factory, OptimismPortal__factory} from "../typechain-types";
import {WithdrawalProvenEvent} from "../typechain-types/OptimismPortal";
import {formatEther} from "ethers/lib/utils";
import axios from "axios";

const RelaySignature = "relayMessage(uint256,bytes32,address,uint256,uint256,bytes)";
const FinalizeETHSignature = "finalizeBridgeETH(bytes32,address,uint256,bytes)";
const FinalizeERC20Signature = "finalizeBridgeERC20(address,bytes32,bytes32,address,uint256,bytes)";
...
async function handleEvent(evm: EVM_CONTEXT, event: WithdrawalProvenEvent) {
    let transaction  = await evm.EVM_PROVIDER.getTransaction(event.transactionHash);
    ...
    if (parsedTransaction.args._tx.target === evm.EVM_MESSENGER) {
        const messengerInterface = L1CrossDomainMessenger__factory.createInterface();
        const relayData = messengerInterface.decodeFunctionData(RelaySignature, parsedTransaction.args._tx.data);

        if (relayData[2] === evm.EVM_STANDARD_BRIDGE) {
            const bridgeInterface = L1StandardBridge__factory.createInterface();

            const finalizeData = bridgeInterface.decodeFunctionData(FinalizeETHSignature, relayData[5]);
            ...
            await sendSlackMessage(warnMessage);
        }
    } else {
...
async function sendSlackMessage(message: string) {
    console.log(message);
    const response = await axios.post(SlackHookURL, {
        text: message,
        headers: {

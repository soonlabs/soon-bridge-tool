    ...
    while (true) {
        try {
            ...
            for (let i = 0; i < failed.length; i++) {
                let item = failed[i];
                let isRelayed = await L1CrossDomainMessenger.successfulMessages(item.topics[1]);
                if (isRelayed) {
                    continue;
                }
...
async function handleEvent(evm: EVM_CONTEXT, event: FailedRelayedMessageEvent) {
    ...
    const message = `warn: failed withdraw transaction: <https://etherscan.io/tx/${event.transactionHash}|${event.transactionHash}>, ready to relay...`;
    await sendSlackMessage(SlackHookURL, message);
    ...
    if (parsedTransaction.args._tx.target === evm.EVM_MESSENGER) {
        ...
        if (relayData[2] === evm.EVM_STANDARD_BRIDGE) {
            ...
            try {
                ...
                const message = `warn: Relay withdraw success. txHash: <https://etherscan.io/tx/${receipt.transactionHash}|${receipt.transactionHash}>`;
                await sendSlackMessage(SlackHookURL, message);
            } catch (e) {
            ...
            await sendSlackMessage(SlackHookURL, `invalid withdraw transaction, invalid relay target`);
        }
    } else {
        await sendSlackMessage(SlackHookURL, `invalid withdraw transaction, invalid bridge target`);
    }
}
